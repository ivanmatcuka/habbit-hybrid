<template>
  <ion-content>
    <slot />
  </ion-content>
  <ion-footer v-if="isDev">
    <ion-toolbar>
      <span class="p-2"> API URL: {{ apiUrl }} </span>
    </ion-toolbar>
  </ion-footer>
</template>

<style lang="scss">
@use '../node_modules/habits-frontend/src/assets/scss/variables.scss' as *;

ion-content {
  --padding-top: #{$spacer * 4};
  --padding-start: #{$spacer * 2};
  --padding-end: #{$spacer * 2};
  --padding-bottom: #{$spacer * 4};
}

ion-content::part(scroll) {
  margin-right: var(--ion-safe-area-right, 0);
  margin-left: var(--ion-safe-area-left, 0);
}
</style>

<script lang="ts">
import { IonContent, IonFooter, IonToolbar } from '@ionic/vue';
import { defineComponent } from 'vue';

export default defineComponent({
  components: { IonContent, IonFooter, IonToolbar },
  computed: {
    apiUrl() {
      return import.meta.env.VITE_API_URL;
    },
    isDev() {
      return import.meta.env.MODE === 'development';
    },
  },
} as Parameters<typeof defineComponent>);
</script>
